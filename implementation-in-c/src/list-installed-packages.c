...
static int _list_dir(const NDKPKGTargetPlatform * targetPlatform, const char * packageInstalledRootDIR, const size_t packageInstalledRootDIRCapacity, const char * platformDIRName, const size_t platformDIRNameCapacity, const bool verbose) {
    ...
    for (;;) {
        ...
        if (lstat(receiptFilePath, &st) == 0 && S_ISREG(st.st_mode)) {
            if (verbose) {
                ret = ndkpkg_installed_info(dir_entry->d_name, targetPlatform, NULL);
...
static int ndkpkg_list_all_installed_packages(const char * packageInstalledRootDIR, const size_t packageInstalledRootDIRCapacity, const bool verbose) {
...
int ndkpkg_list_the_installed_packages(const NDKPKGTargetPlatform * targetPlatform, const bool verbose) {
    ...
    if (targetPlatform == NULL) {
        return ndkpkg_list_all_installed_packages(packageInstalledRootDIR, packageInstalledRootDIRCapacity, verbose);
    }
